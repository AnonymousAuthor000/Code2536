...
TfLiteRegistration* Register_akxxbp_REF() {
  static TfLiteRegistration r = {
      nullptr, nullptr, akxxbp::Prepare,
      akxxbp::Eval<akxxbp::kReference>};
  return &r;
}

TfLiteRegistration* Register_akxxbp_GENERIC_OPT() {
  static TfLiteRegistration r = {
      nullptr, nullptr, akxxbp::Prepare,
      akxxbp::Eval<akxxbp::kGenericOptimized>};
  return &r;
}

TfLiteRegistration* Register_akxxbp() {
  // TODO(ahentz): It turns out the two versions of akxxbp are almost
  // identical, so we should consider removing one.
  return Register_akxxbp_GENERIC_OPT();
}
